...
def scrape_google_maps(query, limit=100, headless=False, job_id=None, merge=True, provider="auto", model=None, hf_token=None, enable_fallback=True):
    ...
    """
    print(f"Starting scraper for query: '{query}' (Limit: {limit}, Headless: {headless}, Merge: {merge})")
    ...
    with sync_playwright() as p:
        ...
        update_job_status(job_id, "active", progress=5, stage="navigating", status_message=f"Connecting to Google Maps for '{query}'...")
        ...
        update_job_status(job_id, "active", progress=10, stage="searching", status_message=f"Typing search query: '{query}'...")
        ...
        update_job_status(job_id, "active", progress=12, stage="searching", status_message="Search submitted. Awaiting Google Maps results...")
        ...
        # Calculate target listings to load: if merging, aim for limit + existing leads
        existing_count = len(existing_ids.get("names", [])) if (merge and existing_xlsx) else 0
        target_to_load = limit + existing_count
        ...
        for attempt in range(max_scroll_attempts):
            ...
            scroll_progress = min(19, 12 + int((min(current_count, target_to_load) / max(1, target_to_load)) * 7))
            update_job_status(
                job_id,
                "active",
                ...
                total=target_to_load,
                status_message=f"Found {current_count} listings on Google Maps..."
            )
            ...
            if current_count >= target_to_load:
                print(f"Reached loading target of {target_to_load} listings ({limit} new + {existing_count} existing).")
                break
        ...
        # Begin extracting details until `limit` NEW leads are scraped
        records = []
        batch_num = 1
        ...
        print(f"\nScroll complete. Beginning details extraction (Goal: {limit} new leads)...")
        update_job_status(
            job_id,
            "active",
            progress=20,
            ...
            current=0,
            total=limit,
            status_message=f"Beginning details extraction (Goal: {limit} new leads)..."
        )
        
        while new_leads_collected < limit:
            if is_job_cancelled(job_id):
                print(f"\n[Notice] Job {job_id} was cancelled by user. Halting scraping loop.")
                browser.close()
                return

            total_cards = page.locator(cards_selector).count()
            
            # If we reached the end of currently loaded cards in DOM, scroll down to load more
            if card_index >= total_cards:
                print(f"Processed all {total_cards} cards loaded so far ({new_leads_collected}/{limit} new leads collected). Scrolling for more...")
                page.evaluate(
                    'const el = document.querySelector(\'div[role="feed"]\'); if (el) { el.scrollBy(0, el.scrollHeight); }'
                )
                random_delay(1.5, 3.0)
                new_total = page.locator(cards_selector).count()
                if new_total <= total_cards:
                    consecutive_scroll_fails += 1
                    if consecutive_scroll_fails >= 3:
                        print("Reached the end of Google Maps results. No more listings found.")
                        break
                else:
                    consecutive_scroll_fails = 0
            ...
            print(f"\nProcessing listing card {card_index + 1} (New collected: {new_leads_collected}/{limit}, Dups skipped: {duplicates_skipped})...")
            card_name = None
            
            try:
                card = page.locator(cards_selector).nth(card_index)
                ...
                # Check if business already exists in database (early deduplication)
                if card_name:
                    clean_name = re.sub(r'[^a-zA-Z0-9]', '', card_name.lower())
                    if clean_name and clean_name in existing_ids["names"]:
                        print(f"Skipping duplicate listing: '{card_name}' (already in database)")
                        duplicates_skipped += 1
                        card_index += 1
                        scrape_progress = 20 + int((new_leads_collected / max(1, limit)) * 50)
                        update_job_status(
                            job_id,
                            "active",
                            ...
                            total=limit,
                            status_message=f"Skipped duplicate: {card_name} (Collected {new_leads_collected}/{limit} new leads)"
                        )
                        render_cli_progress(new_leads_collected, limit, prefix="Scraping Details", status=f"Duplicate: {card_name}")
                        continue
                        
                # Click the card to open place details
                card.click(timeout=5000)
            except Exception as e:
                print(f"Warning: Could not click card {card_index + 1}: {e}")
                card_index += 1
                continue
            ...
            details_panel = page.locator(details_panel_selector)
            try:
                details_panel.wait_for(state="visible", timeout=8000)
            except Exception:
                print(f"Warning: Place details panel did not open for card {card_index + 1}.")
                card_index += 1
                continue
                
            # Verify the details panel matches the clicked card name
            if card_name:
                for _ in range(5):
                    try:
                        active_header = details_panel.locator('h1.DUwDvf').first
                        if active_header.count() > 0:
                            header_text = active_header.inner_text().strip()
                            if header_text and (card_name.lower() in header_text.lower() or header_text.lower() in card_name.lower()):
                                break
                    except Exception:
                        pass
                    time.sleep(0.3)
                    
            # Scroll details panel to trigger lazy loading of address, website, hours
            for _ in range(2):
                page.evaluate(
                    'const el = document.querySelector(\'div[role="main"][aria-label]\'); if (el) { el.scrollBy(0, 1000); }'
                )
            ...
            if record and record.get("name"):
                # Secondary deduplication check after full extraction
                is_dup = False
                if record.get("phone"):
                ...
                if not is_dup and record.get("maps_url"):
                    clean_url = str(record["maps_url"]).split('?')[0].rstrip('/')
                    if clean_url in existing_ids["urls"]:
                        is_dup = True
                if not is_dup and record.get("name"):
                    clean_extracted_name = re.sub(r'[^a-zA-Z0-9]', '', str(record["name"]).lower())
                    if clean_extracted_name in existing_ids["names"]:
                        is_dup = True
                ...
                if is_dup:
                    ...
                    update_job_status(
                        ...
                        current=new_leads_collected,
                        total=limit,
                        status_message=f"Skipped duplicate: {record['name']} (Collected {new_leads_collected}/{limit} new leads)"
                    )
                    render_cli_progress(new_leads_collected, limit, prefix="Scraping Details", status=f"Duplicate: {record['name']}")
                    continue
                    
                records.append(record)
                new_leads_collected += 1
                
                # Register in local identifiers set to prevent intra-scrape duplicates
                if record.get("name"):
                ...
                print(f"Scraped NEW Lead ({new_leads_collected}/{limit}): '{record.get('name')}' (Rating: {record.get('rating')}, Reviews: {record.get('reviews_count')})")
                ...
                update_job_status(
                    ...
                    current=new_leads_collected, 
                    total=limit,
                    status_message=f"Scraped {new_leads_collected}/{limit} new leads: {record.get('name')}"
                )
                render_cli_progress(new_leads_collected, limit, prefix="Scraping Details", status=f"{record.get('name')}")
                
                # Save batch when reaching 20 records
                if len(records) == 20:
                    save_batch(records, batch_num)
                    records = []
                    batch_num += 1
            else:
                fallback_name = card_name or f"Listing #{card_index}"
                print(f"Skipping listing '{fallback_name}': Could not extract valid details.")
                continue
...
def extract_details(details_panel, page):
    ...
    try:
        ...
        if addr_el.count() > 0:
            raw_addr = addr_el.inner_text().strip()
            # Clean leading unicode icons (like \ue0c8) and extra whitespace
            record["address"] = re.sub(r'^[\s\ue000-\uf8ff]+', '', raw_addr).strip()
    except Exception:
        pass
    ...
    if record.get("website") and (not instagram_link or not facebook_link):
        try:
            print(f"Scraping social profiles from website: {record['website']}...")
            ...
            print(f"Warning: Failed website social scrape: {e}")
